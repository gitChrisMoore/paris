  ...
  return (
    ...
    <>
      ...
      <LeftNavDrawer isNavOpen={isNavOpen} setIsNavOpen={setIsNavOpen}>
        {/* Brand */}
        <h2 className=" font-bold mx-2 py-2 leading-snug text-slate-800">{'Brands'}</h2>
        {brandNavs.map((t) => {
          return <LeftNavDrawerItem key={t.name} title={t.name} to={t.path} />;
        })}
        {/* Products */}
        <h2 className=" font-bold mx-2 py-2 leading-snug text-slate-800">{'Products'}</h2>
        {productNavs.map((t) => {
          return <LeftNavDrawerItem key={t.name} title={t.name} to={t.path} />;
        })}
        {/* Auth */}
        <h2 className=" font-bold mx-2 py-2 leading-snug text-slate-800">{'Retailer'}</h2>
        {authNavs.map((t) => {
          return <LeftNavDrawerItem key={t.name} title={t.name} to={t.path} />;
        })}
      </LeftNavDrawer>
